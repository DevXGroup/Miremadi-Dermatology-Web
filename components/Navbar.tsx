      ...
      <div className="hidden lg:flex items-center gap-8">
        {navLinks.map((link) => (
          <a
            key={link.name}
            href={link.href}
            className={`text-[10px] font-bold uppercase tracking-[0.2em] transition-all hover:text-indigo-600 relative py-1 ${
              activeSection === link.id ? 'text-indigo-600' : 'text-slate-400'
            }`}
          >
            {link.name}
        ...
        <div className="flex items-center gap-3">
           <a href="https://www.instagram.com/miremadi___dermatology/?hl=en" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-indigo-600 transition-colors">
           ...
           <a href="https://www.facebook.com/miremadidermatology/" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-indigo-600 transition-colors">
        ...
        <button className="bg-slate-900 text-white px-8 py-3 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-indigo-600 transition-all transform hover:-translate-y-0.5 shadow-xl active:translate-y-0 ml-4">
          Appointment
        </button>
      </div>
      ...
      {isMenuOpen && (
        <div className="absolute top-full left-0 right-0 bg-white/98 backdrop-blur-2xl border-b border-slate-100 p-10 flex flex-col gap-8 lg:hidden animate-fade-in shadow-2xl">
          ...
          <div className="flex justify-between items-center py-4">
             ...
             <p className="text-xs text-indigo-500 font-bold">Hablamos Español</p>
          </div>
        </div>
      )}
    </nav>
